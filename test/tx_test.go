...
Loop:
	...
	tx, err := c.Transfer("TNzLXcYS4XKvSH3k3GUSUT87zq9a2wnQjr", "TS16Z7BhGJtnR4p6tGmQDVggGr6u4z2222", 100000000)
	if err != nil {
		fmt.Println(111)
		t.Fatal(err)
	}
	signTx, err := sign.SignTransaction(tx.Transaction, "f3387d4a6430e83ac096d680acf60944023876bf9d28159db205e6c16db27a71")
	if err != nil {
		fmt.Println(222)
		t.Fatal(err)
	}
	blockbefor, err := c.GRPC.GetNowBlock()
	fmt.Println("blockbefor：" + common.BytesToHexString(blockbefor.GetBlockid()))
	rs := []rune(common.BytesToHexString(blockbefor.GetBlockid()))
	sr := string([]rune(rs)[len([]rune(rs))-2:])
	if err != nil {
		fmt.Println(333)
		t.Fatal(err)
	}
	for true {
		...
		nowBlock, err := c.GRPC.GetNowBlock()
		if err != nil {
			fmt.Println(444)
			t.Fatal(err)
		}
		rsn := []rune(common.BytesToHexString(nowBlock.GetBlockid()))
		nsr := string([]rune(rsn)[len([]rune(rs))-2:])
		if sr != nsr {
			matchString, err := regexp.MatchString(`^[A-Za-z]+$`, nsr)
			if err != nil {
				fmt.Println(555)
				t.Fatal(err)
			}
			...
			if !matchString && !num {
				...
				fmt.Println("nowBlock：" + common.BytesToHexString(nowBlock.GetBlockid()))
				goto Loop
			}
			goto Loop
		}
	}
	fmt.Println("Txid：" + common.BytesToHexString(tx.GetTxid()))
	blockaffter, err := c.GRPC.GetNowBlock()
	fmt.Println("blockaffter：" + common.BytesToHexString(blockaffter.GetBlockid()))
}
...
func Test_GetBalance(t *testing.T) {
	...
	blockbefor, err := c.GRPC.GetNowBlock()
	fmt.Println(common.BytesToHexString(blockbefor.GetBlockid()))
	acc, err := c.GetTrxBalance("TNzLXcYS4XKvSH3k3GUSUT87zq9a2wnQjr")
	if err != nil {
		t.Fatal(err)
	}
	...
	blockaffter, err := c.GRPC.GetNowBlock()
	fmt.Println(common.BytesToHexString(blockaffter.GetBlockid()))
